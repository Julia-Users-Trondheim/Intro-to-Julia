...
# ╔═╡ 15bd2b1a-f948-4365-97a7-8cd82150857b
begin
	using PlutoUI
...
# ╔═╡ 0b9ffc10-0434-11f0-394e-95eb3fc1c3f8
md"""
# Solving a classification problem with neural networks: The effect of regularization
"""

# ╔═╡ a0558625-d9c4-4860-b307-443666906a9f
md"""
### Dependencies ###
"""

# ╔═╡ 773cae60-1b83-4b68-8fce-c6edfa2dfbbf


# ╔═╡ 6d2e406a-bc3f-4bb0-9c04-5359b943408e
md"""
## Loading the data set ##
"""

# ╔═╡ 95762356-9529-4604-8b70-627d20060120
dataset = CSV.read(download("https://raw.githubusercontent.com/mehdizouari/Moon-Data-Classification/refs/heads/master/train.csv"), DataFrame, header=false)

# ╔═╡ f59ecc0a-7db7-4aee-bce1-85bd5fe23fc4
begin
	X = Array(dataset[:, 2:end]);
	Y = dataset.Column1;
	scatter(X[:, 1], X[:, end], group = Y)
end

# ╔═╡ c2c14213-f9ff-43cc-ad90-6738d4842318
md"""
## What we want is to train a classifier that can separate the two classes :-) ##
"""

# ╔═╡ 8862672d-6991-472a-9f6d-8770c6c179d8
md"""
First we can create a multi-layer perceptron using Flux.jl
"""

# ╔═╡ 76a0b168-65c4-4de5-85f5-704c20a6388c
md"""
Then, we can set up a loss function that is suitable for classification
"""

# ╔═╡ 6f546ddf-5be0-4b78-81fb-37698e7ba60e
begin
pen_l2(x::AbstractArray) = sum(abs2, x)/2
loss(model, x, y, λ) = Flux.binarycrossentropy(model(X'), Y') + λ * sum(pen_l2, Flux.trainables(model))
end

# ╔═╡ bbcebc3e-b4b5-42eb-90aa-a151acec8b4b
md"""
Now we can create our training loop
"""

# ╔═╡ e8e3965e-d1d5-4e82-965b-3538202bd1cd
function train_nn(λ, model)
...
# ╔═╡ 49c64f21-cc72-4475-a4b3-bcdca90c7a4b
begin
grid_x = grid_y = range(-1.5, stop=2.5, length=50)
grid = hcat(vec([x for x in grid_x, y in grid_y]), vec([y for x in grid_x, y in grid_y]))';
end

# ╔═╡ 2d4607dd-ac2e-485f-87dd-c2005bdecb34
@bind λ Slider(0.0:0.0001:0.05, default=0.0001, show_value=true)


# ╔═╡ f602fd11-1122-4c28-acb0-6329eeacbe4e
begin
model = Chain(
		Dense(2, 10, tanh),
		Dense(10, 10, tanh),
...
train_nn(λ, model)	
println(loss(model, X, Y, 0.0))
preds = model(grid)
contourf(grid_x, grid_y, reshape(preds, (50, 50))';
		 title="Effect of Regularization λ= $λ", xlabel="x1", ylabel="x2",
		 legend=false)
scatter!(X[:, 1], X[:, 2], group = Y, markerstrokewidth=0.2, ms=3)
end
...
manifest_format = "2.0"
project_hash = "7390150b780f2988d95c2f0cb11d2996c80d8790"
